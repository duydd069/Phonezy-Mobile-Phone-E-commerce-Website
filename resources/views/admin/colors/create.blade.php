...
<div class="container-fluid p-3">
  <h3 class="mb-3">Thêm màu sắc mới</h3>
  ...
  <form action="{{ route('admin.colors.store') }}" method="POST" class="row g-3" novalidate>
    ...
    <div class="col-md-6">
      ...
      <small class="text-muted">VD: Đỏ, Xanh dương, Đen, Trắng...</small>
    </div>

    <div class="col-md-6">
      <label class="form-label">Mã màu (Hex Code)</label>
      <input type="color" name="hex_code_color" id="hex_code_color" class="form-control form-control-color" value="{{ old('hex_code', '#4CD964') }}" style="width: 60px; height: 38px; cursor: pointer;">
      <input type="text" name="hex_code" id="hex_code" class="form-control mt-2" value="{{ old('hex_code') }}" placeholder="#FF0000" maxlength="7" pattern="^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$">
      <small class="text-muted">Định dạng: #RRGGBB (VD: #FF0000 cho màu đỏ) hoặc chọn từ color picker</small>
    </div>
